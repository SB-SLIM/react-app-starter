...
- `packages/` — shared libraries built with `tsup`, consumed by apps
  ...
  - `@sb-codex/acl` — RBAC. **Permission-based** access via `resource:action` strings (`permissions.ts`): `PERMISSIONS`, `Permission`, server-only `ROLE_PERMISSIONS` map, `permissionsFor(role)`, `can(role, perm)`, and the `requirePermission(perm)` procedure factory — the canonical feature gate. Roles (`MemberRole` in `@sb-codex/api-contracts`) are `owner | admin | manager | commercial | member`. Legacy rank API kept: `hasRole`, `ROLE_HIERARCHY`, `enforceRole(allowed)`, `adminProcedure`, `ownerProcedure`. React client (`./client`): `AclProvider` (holds resolved `permissions: string[]`), `Can`, `usePermission`, `useAcl`; plus back-compat `useRole`, `AccessGuard`. **Server is the source of truth** — the role→permission map never ships to the browser; the client receives resolved strings from `members.me` and feeds `AclProvider`.
- `apps/` — end-user applications
  - `admin` — React 19 + Vite + Tailwind v4 + TanStack Router/Query → `hub-admin.slimbouchoucha.tn`
  - `superadmin` — React 19 + Vite — platform super admin → `hub-superadmin.slimbouchoucha.tn`. Gated on the platform-level `user.is_super_admin` flag (surfaced on the better-auth session via `additionalFields`, `input: false`); a non-super-admin who signs in is signed out with an "not authorized" error. Bootstrap the first super admin with `pnpm seed:superadmin` (`SUPERADMIN_PASSWORD` env required) — there is no other path since promotion needs an existing super admin. This **platform** axis is separate from the per-workspace `MemberRole` permission system.
